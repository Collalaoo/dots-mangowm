...
Item {
    ...
    readonly property var monitor: HyprlandData.monitorFor(root.QsWindow.window?.screen)
    ...
    WheelHandler {
        onWheel: (event) => {
            if (event.angleDelta.y < 0)
                HyprlandData.dispatch(`hl.dsp.focus({workspace = "r+1"})`);
    ...
    MouseArea {
        ...
        onPressed: (event) => {
            if (event.button === Qt.BackButton) {
                HyprlandData.dispatch(`hl.dsp.workspace.toggle_special("special")`);
            } 
        }
    }
    ...
    Grid {
        ...
        Repeater {
            ...
            Button {
                ...
                onPressed: HyprlandData.dispatch(`hl.dsp.focus({ workspace = ${workspaceValue}})`)
